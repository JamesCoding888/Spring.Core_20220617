...
public class MyLoggerAspect {
		...
		public void afterReturningAdvice(JoinPoint joinPoint, Object result) {
			String methodName = joinPoint.getSignature().getName(); // 取得連接點的方法簽章名稱
			System.out.printf("返回通知 - 方法名稱: %s\n 回傳值", methodName, result);	
		}
		
		// 異常通知 Advice: 可以設定 throwing 來接收異常物件
		@AfterThrowing(value = "pt()", throwing = "ex")
		public void afterThrowingAdvicec(JoinPoint joinPoint, Throwable ex) { // 使用Throwable 因包含 Error & Exception
			String methodName = joinPoint.getSignature().getName(); // 取得連接點的方法簽章名稱
			System.out.printf("異常通知 - 方法名稱: %s 例外: \n ", methodName, ex);	
		}
		*/
		// 環繞通知
		...
		public Object aroundAdvice(ProceedingJoinPoint joinPoint) {
			Object result = null;
			String methodNamee = joinPoint.getSignature().getName();
			Object[] args = joinPoint.getArgs();
			try {
				// 1. 前置通知
				System.out.printf("環繞通知: 前置通知 - 方法名稱: %s 參數: %s\n", methodNamee, Arrays.toString(args));
				// 2. 執行業務邏輯 (重要!!! 一定要加入）
				...
				// 5. 後製通知
				System.out.printf("環繞通知: 後製通知 - 方法名稱: %s\n", methodNamee);
			}
			return result;
		}
